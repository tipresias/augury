...
gcloud beta run deploy augury \
  ...
  --region us-central1 \
  --platform managed \
  --update-env-vars AFL_DATA_SERVICE_TOKEN=${AFL_DATA_SERVICE_TOKEN},PYTHON_ENV=production,DATA_SCIENCE_SERVICE_TOKEN=${DATA_SCIENCE_SERVICE_TOKEN},AFL_DATA_SERVICE=${AFL_DATA_SERVICE},TIPRESIAS_APP_TOKEN=${TIPRESIAS_APP_TOKEN}
